        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2, values_only=False):
                if row[0].value == selected_item:
                    item_found = True
                    # Update LastCount with the current NewCount before changing it
                    row[1].value = row[2].value
                    # Perform the operation on NewCount
                    new_value = row[2].value or 0
                    new_value = new_value + input_value if operation == 'add' else new_value - input_value
                    row[2].value = new_value
                    # Save the workbook and update the Treeview and log view
                    workbook.save(workbook_path)
                    update_treeview()
