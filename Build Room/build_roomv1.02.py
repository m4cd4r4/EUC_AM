...
if os.path.exists(workbook_path):
    ...
    workbook.create_sheet('In_Transit')  # Sheet 5
    workbook.create_sheet('In_Transit_Timestamps')  # Sheet 6
    workbook.create_sheet('All_SANs')  # Sheet 7
    # Initialize headers
    workbook['4.2_Items'].append(["Item", "LastCount", "NewCount"])
    workbook['4.2_Timestamps'].append(["Item", "Action", "SAN Number", "Timestamp"])  # Corrected sheet name
    workbook['BR_Items'].append(["Item", "LastCount", "NewCount"])
    workbook['BR_Timestamps'].append(["Item", "Action", "SAN Number", "Timestamp"])  # Adjusted order
    workbook['In_Transit'].append(["Item", "LastCount", "NewCount"])  # Modify as needed
    workbook['In_Transit_Timestamps'].append(["Item", "Action", "SAN Number", "Timestamp"])  # Adjusted order
    workbook['All_SANs'].append(["Item", "SAN Number", "Timestamp"])  # Adjusted order
...
def log_change(item, action, target_sheet, san_number=""):
    ...
    if target_sheet.title in ['BR_Timestamps', 'In_Transit_Timestamps']:
    ...
    elif target_sheet.title == 'All_SANs':
