...
# Items to be added to the sheets
items = [
    "Desktop Mini G9",
    ...
    "Wired Keyboard",
    "Wired Mouse",
    "Wireless Headset Poly",
    "Wireless KB & Mouse"
]

# Path to the workbook
workbook_path = Path('EUC_Perth_Assets.xlsx')

# Check if the workbook exists at the path
if workbook_path.exists():
    ...
    four_two_sheet = workbook.active
    four_two_sheet.append(["Item", "LastCount", "NewCount"])
    for item in items:
        four_two_sheet.append([item])

    workbook.create_sheet('4.2 Timestamps')
    timestamps_sheet = workbook['4.2 Timestamps']
    timestamps_sheet.append(["Timestamp", "Item", "Action", "SAN Number"])

    workbook.create_sheet('BR Items')
    br_sheet = workbook['BR Items']
    br_sheet.append(["Item", "LastCount", "NewCount"])
    for item in items:
        br_sheet.append([item])

    workbook.create_sheet('BR Timestamps')
    br_timestamps_sheet = workbook['BR Timestamps']
    br_timestamps_sheet.append(["Timestamp", "Item", "Action", "SAN Number"])

    workbook.create_sheet('Project Designated Items')
    pdi_sheet = workbook['Project Designated Items']
    pdi_sheet.append(["Item", "LastCount", "NewCount"])

    workbook.create_sheet('Project Designated Timestamps')
    pdt_sheet = workbook['Project Designated Timestamps']
    pdt_sheet.append(["Timestamp", "Item", "Action", "SAN Number"])

    workbook.create_sheet('All SANs')
    all_sans_sheet = workbook['All SANs']
    all_sans_sheet.append(["SAN Number", "Item", "Timestamp"])
...
class SANInputDialog(tk.Toplevel):
    ...
    def on_submit(self):
        ...
        if san_number:
            self.result = san_number  # Dash removed between prefix and ticket number
            self.destroy()
        else:
    ...
    if not san_required and not headset_required:
        for row in item_sheet.iter_rows(min_row=2):
            if row[0].value == selected_item:
                ...
                # Move the log_change call outside of the for loop
                log_change(selected_item, operation, input_value, "", timestamp_sheet)
                break  # Since we found the item and logged the change, break out of the loop
