...
def update_count(operation):
    ...
    if selected_item and "Headset" in selected_item:
        ...
        if serial_number:
            ...
            # Write the serial number and timestamp to the "headsets" sheet
            headsets_sheet = workbook['Headsets']
            ...
            if servicenow_number:
                # Write the ServiceNow number to the next column of the "headsets" sheet
                headsets_sheet.cell(row=next_row, column=2, value=servicenow_number)
            else:
                # ServiceNow dialog canceled; Prompt for notes and write to column C
                note = sd.askstring("Note", "Please log a note:", parent=root)
                headsets_sheet.cell(row=next_row, column=3, value=note)
                
            # Write the current timestamp to column D
            headsets_sheet.cell(row=next_row, column=4, value=current_timestamp)
