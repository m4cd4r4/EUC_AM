# Fix 6-digit limit on SAN Input function (Currently no min/max characters)

# Build Room\build_roomv2.11.py
...
class SANInputDialog(tk.Toplevel):
    ...
    def on_submit(self):
        ...
        # Check if the length of the SAN number is exactly 6
        if len(san_number) == 6:
            self.result = san_number
            self.destroy()
        else:
            tk.messagebox.showerror("Error", "Please input exactly 6 numbers for the SAN.", parent=self)
            self.entry.delete(0, tk.END)  # Clear the entry for new input
