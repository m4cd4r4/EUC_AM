...
for col in columns:
    ...
    tree.column(col, anchor=tk.W, minwidth=0, width=150, stretch=False)  # Adjust width as necessary
tree.pack(expand=True, fill="both", padx=10, pady=10)
tree.bind('<ButtonRelease-1>', lambda e: entry_value.focus())  # Focus on entry field when an item is selected

# Load the logo image and place it at the top right corner above the treeview
logo_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'santos.png')
logo_image = tk.PhotoImage(file=logo_path)
# Reduce the logo size
logo_image = logo_image.subsample(5, 5)  # subsample by a factor of 5 to reduce the size
# Place the logo above the treeview to the right
