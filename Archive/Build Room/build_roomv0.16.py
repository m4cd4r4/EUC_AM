...
if not log_sheet['A1'].value:
    log_sheet.append(["Timestamp", "Item", "Action"])

# Save the workbook if it was newly created
...
for col in columns:
    ...
    tree.column(col, anchor=tk.W, width=200, stretch=False)  # Ensure width is enough for data
tree.pack(expand=True, fill="both", padx=10, pady=20)  # Provide space between the Treeview and the spreadsheet view
