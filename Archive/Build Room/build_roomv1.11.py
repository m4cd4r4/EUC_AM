# Refresh the logview after each operation
# Arrange the logview in "newest first" order of the 4th column, "Timestamp"
...
# Define all_sans_sheet globally
...
def is_san_unique(san_number):
    global all_sans_sheet  # Access the global variable
...
def log_change(item, action, san_number="", timestamp_sheet=None):
    ...
    try:
        if timestamp_sheet is not None:
            ...
            # Assuming the order of columns in the timestamp sheet is: Timestamp, Item, Action, SAN Number
            timestamp_sheet.append([timestamp, item, action, san_number])
            workbook.save(workbook_path)
            update_log_view()  # Refresh the log view after each log change
...
def update_count(operation):
    try:
        ...
        if selected_item:
            ...
            # Correctly initiate SAN input prompt based on the entered number
            san_inputs = []
            for _ in range(abs(input_value)):
                ...
                if san_number not in san_inputs:  # Check if SAN is already in the list for current operation
                    if is_san_unique(san_number):  # Check for uniqueness
                        san_inputs.append(san_number)  # Add the unique SAN to the list
                    else:
                        tk.messagebox.showerror("Duplicate SAN", "This SAN number is already recorded in the system. Please reenter a unique SAN.")
                        continue  # Skip the current iteration and prompt for SAN input again
                else:
                    tk.messagebox.showerror("Duplicate Entry", "This SAN number has already been entered. Please enter a different SAN.")
                    continue  # Skip the current iteration and prompt for SAN input again

            # Now append the SANs to the 'All SANs' sheet
            ...
            # Update the counts for the selected item
            for row in item_sheet.iter_rows(min_row=2):
                if row[0].value == selected_item:
                    row[1].value = row[2].value or 0
                    if operation == 'add':
                        row[2].value = (row[2].value or 0) + len(san_inputs)  # Only increase by the number of unique SANs entered
                    elif operation == 'subtract':
                        row[2].value = max((row[2].value or 0) - len(san_inputs), 0)  # Only decrease by the number of unique SANs entered
        ...
        workbook.save(workbook_path)
        update_treeview()
        update_log_view()  # Refresh the log view after updating the count
    ...
    for _ in range(abs(input_value)):
        while True:
            ...
            if is_san_unique(san_number):
                # Change the order of append to match the required: SAN Number, Item, Time
...
# Way too many comments, several functions can be refactored, improve pythonic readability 