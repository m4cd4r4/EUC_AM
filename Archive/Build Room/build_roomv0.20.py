...
button_subtract = ctk.CTkButton(entry_frame, text="-", command=lambda: update_count('subtract'), text_font=button_font)
button_subtract.pack(side='left', padx=5)

# Entry Value using CustomTkinter
entry_value = ctk.CTkEntry(entry_frame, width=200, text_font=button_font)
entry_value.pack(side='left')

# Add Button using CustomTkinter
button_add = ctk.CTkButton(entry_frame, text="+", command=lambda: update_count('add'), text_font=button_font)
