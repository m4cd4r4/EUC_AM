...
def update_count(operation):
    ...
    if selected_item:
        san_and_servicenow_numbers = []  # List to store tuples of (SAN, ServiceNow)
        ...
        if input_value.isdigit():
            ...
            if any(requirement in selected_item for requirement in ["G8", "G9", "G10", "Headset"]) and input_value > 1:
                for _ in range(input_value):
                    if "Headset" in selected_item:
                        ...
                        if servicenow_number is None:  # Giving the option to cancel operation
                            return
                        san_and_servicenow_numbers.append(("", serial_number, servicenow_number))
                    else:
                        san_number = show_san_input()
                        if not san_number:  # Giving the option to cancel operation
                            return
                        servicenow_number = servicenow_number_input()
                        if not servicenow_number:  # Giving the option to cancel operation
                            return
                        san_and_servicenow_numbers.append((san_number, "", servicenow_number))
            
            # Now you have a list of tuples each containing (san_number, serial_number, servicenow_number)
            # You can adjust logic below to make use of this information
            item_sheet = workbook[current_sheets[0]]
            ...
            for row in item_sheet.iter_rows(min_row=2):
                if row[0].value == selected_item:
                    ...
                    # Log operation for each SAN/ServiceNow
                    for san_number, serial_number, servicenow_number in san_and_servicenow_numbers:
                        log_change(selected_item, operation, 1, san_number=san_number, serial_number=serial_number, servicenow_number=servicenow_number, timestamp_sheet=timestamp_sheet)
